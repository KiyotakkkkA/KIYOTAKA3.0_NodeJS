...
function HomeworkProccessing(text, ChatId){
    ...
    switch (text) {
        ...
        case ("✅ Добавить ДЗ"):
            ...
            _bot.BotMsg(ChatId, `${spec_symbols["SB_write"]} По какому предмету? (воспользуйтесь опцией 'ответить' на это сообщение)`)
            break

        case ("🖋️ Редактировать ДЗ"):

            homeW.knowHomework(ChatId)
            _bot.BotMsg(ChatId, `${spec_symbols["SB_edit"]} Выбери ID нужной записи (воспользуйтесь опцией 'ответить' на это сообщение)`)
            break

        case ("🗑 Удалить ДЗ"):

            homeW.knowHomework(ChatId)
            _bot.BotMsg(ChatId, `[🗑] Выбери ID нужной записи (воспользуйтесь опцией 'ответить' на это сообщение)`)
            break
    }
}
