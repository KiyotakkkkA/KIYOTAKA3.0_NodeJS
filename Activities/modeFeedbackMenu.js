...
function feedbackActivity(text, ChatId, msg) {
    if (text == "Обратная связь"){
        _bot.BotMsg(ChatId, spec_symbols['SB_write'] + " Оставьте вашу жалобу/предложение, при возможности прикрепите фотографию (ответьте на это сообщение)")
    }
    if (msg.reply_to_message){
        if (msg.reply_to_message.text == spec_symbols['SB_write'] + " Оставьте вашу жалобу/предложение, при возможности прикрепите фотографию (ответьте на это сообщение)"){
